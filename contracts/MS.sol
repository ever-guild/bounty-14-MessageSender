...
contract MS {
  ...
  constructor(address _MSaddressesNext, address _MSaddressesPrev, address _root, uint32 _salt, uint8 _parralel_request_count, uint8 _position) public {
    tvm.accept();
    ...
    settings.total_threads = _parralel_request_count * Constants.ms_group_members_count;
  ...
  function destroy_sender() public {
    require(msg.sender == settings.prev || msg.sender == settings.next || msg.sender == address(this));
    tvm.accept();
    selfdestruct(settings.trinity_root);
  }
}
