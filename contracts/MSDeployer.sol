...
contract MSDeployer is Utils {
    ...
    constructor (uint8 parralel_request_per_ms_group) public {
      ...
      for (uint8 i = 0; i < salts.length; i++) {
        address _addr = new MS{
          ...
          bounce: true,
          varInit: {}
        }(_getAddressForSalt(salts[(i + 1) % 3]), _getAddressForSalt(salts[(i + 2) % 3]), root, salts[i], parralel_request_per_ms_group, i);
      }
    ...
    function _getAddressForSalt(uint32 _salt) internal view returns (address) {
      ...
      uint256 _addr = tvm.stateInitHash(tvm.hash(saltedCode), tvm.hash(_data), saltedCode.depth(), _data.depth());
  ...
  function _buildDataCode(uint32 _salt) internal view returns (TvmCell) {
    TvmBuilder salt;
    salt.store(_salt);
    return tvm.setCodeSalt(MSCode, salt.toCell());
  }
